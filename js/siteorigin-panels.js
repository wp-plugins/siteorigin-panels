...
( function( $, _, panelsOptions ){
    ...
    panels.model.widget = Backbone.Model.extend( {
        ...
        clone: function( cell, options ){
            ...
            // Create a deep clone of the values from this parent widget
            clone.set( 'values', JSON.parse( JSON.stringify( this.get('values') ) ), { silent: true } );
    ...
    panels.view.cell = Backbone.View.extend( {
        ...
        initSortable: function(){
            ...
            var builderID = this.row.builder.$el.attr('id');
            ...
            this.widgetSortable = this.$el.find('.widgets-container').sortable( {
                ...
                helper: function(e, el){
                    var helper = el.clone().css('width', el.outerWidth()).addClass('widget-being-dragged');
        ...
        setCells: function(cells){
            ...
            if( this.cells.length == 0 ) {
                ...
                if(cells.length > this.cells.length) {
                    ...
                    for( var i = 0; i < cells.length - this.cells.length; i++ ) {
                        var cell = new panels.model.cell({
    ...
    panels.view.row = Backbone.View.extend( {
        ...
        /**
         * Initialize the resizable instances
         */
        ...
        resize: function(e){
            ...
            this.$el.find( '.so-cells, .so-cells .cell' ).css( 'height', 'auto' );
            ...
            this.$el.find( '.so-cells, .so-cells .cell' ).css( 'height',  Math.max( maxHeight, 70 ) );
        },
    ...
    panels.view.builder = Backbone.View.extend( {
        ...
        events: {
            ...
            'mouseenter *[data-tooltip]': 'tooltipShow',
            'mouseleave *[data-tooltip]': 'tooltipHide'
        },
        ...
        render: function(){
            ...
            this.$el.attr( 'id', 'siteorigin-panels-builder-' + this.cid );
            return this;
        },
        ...
        attachToEditor: function(){
            ...
            metabox.find('.so-switch-to-standard').click(function(){
                // Switch back to the standard editor
        ...
        /**
         * Show the tooltip for the currently hovered item.
         * @param e
         */
        tooltipShow: function(e){
            var $$ = $(e.currentTarget);
            var tooltip = $( '<div class="so-panels-tooltip"></div>' ).appendTo( $$ ).html( $$.attr( 'data-tooltip' ) ).append( $( '<div class="pointer"></div>' ) );

            tooltip.css( {
                bottom: $$.outerHeight() + 8,
                left: ( $$.outerWidth() / 2 ) - ( tooltip.outerWidth() / 2 )
            } );
        ...
        /**
         * Hide the tooltip for the currently hovered item.
         * @param e
         */
    ...
    panels.dialog.prebuilt = panels.view.dialog.extend( {
        ...
        events: {
            ...
            'click .so-toolbar .so-insert' : 'insertClickHandler',
            'keyup .so-sidebar-search' : 'searchHandler'
        },
    ...
    panels.dialog.row = panels.view.dialog.extend( {
        ...
        events: {
            ...
            'click .row-set-form .so-button-row-set': 'setCellsFromForm'
        },
            ...
            _.each(this.row.cells, function(cell, i){
                ...
                newCell.find('.preview-cell-weight').click(function(ci){
                    ...
                    rowPreview.find('.preview-cell-weight').each( function(){
                        var $$ = $(this).hide();
                        $('<input type="text" class="preview-cell-weight-input" />')
        ...
        setCellsFromForm: function(){
            ...
            if( f.cells == NaN || f.ratio == NaN || f.cells == 0 ) return [ 1 ];
    ...
    $.fn.soPanelsSetupBuilderWidget = function () {
        ...
        return this.each(function(){
            ...
            if( isWidget ) {
                ...
                // Remove the dialog opener, this is already being displayed in a page builder dialog.
                $$.find( '.siteorigin-panels-display-builder').remove();
            }
