...
define('SITEORIGIN_PANELS_VERSION', '2.0-beta1');
...
function siteorigin_panels_admin_enqueue_scripts($prefix) {
	...
	if ( ( $screen->base == 'post' && in_array( $screen->id, siteorigin_panels_setting('post-types') ) ) || $screen->base == 'appearance_page_so_panels_home_page' || $screen->base == 'widgets') {
		...
		wp_enqueue_script( 'so-panels-admin', plugin_dir_url(__FILE__) . 'js/siteorigin-panels.min.js', array( 'jquery', 'jquery-ui-resizable', 'jquery-ui-sortable', 'jquery-ui-draggable', 'underscore', 'backbone' ), SITEORIGIN_PANELS_VERSION, true );
		wp_enqueue_script( 'so-panels-admin-styles', plugin_dir_url(__FILE__) . 'js/siteorigin-panels-styles.min.js', array( 'so-panels-admin', 'jquery', 'underscore', 'backbone', 'wp-color-picker' ), SITEORIGIN_PANELS_VERSION, true );

		if( $screen->base != 'widgets' ) {
			// We don't use the history browser and live editor in the widgets interface
			wp_enqueue_script( 'so-panels-admin-history', plugin_dir_url(__FILE__) . 'js/siteorigin-panels-history.min.js', array( 'so-panels-admin', 'jquery', 'underscore', 'backbone' ), SITEORIGIN_PANELS_VERSION, true );
			wp_enqueue_script( 'so-panels-admin-live-editor', plugin_dir_url(__FILE__) . 'js/siteorigin-panels-live-editor.min.js', array( 'so-panels-admin', 'jquery', 'underscore', 'backbone' ), SITEORIGIN_PANELS_VERSION, true );
		}
		...
		wp_localize_script( 'so-panels-admin', 'soPanelsOptions', array(
			...
			'loc' => array(
				...
				'confirm_use_builder' => __("Would you like to copy your page's existing content to Page Builder?", 'siteorigin-panels'),
...
function siteorigin_panels_get_widgets(){
	...
	foreach($wp_widget_factory->widgets as $class => $widget_obj) {
		$widgets[$class] = array(
			...
			'installed' => true,
			'groups' => apply_filters( 'siteorigin_panels_widget_groups_'.$class, apply_filters('siteorigin_panels_widget_groups', array() ) ),
		);
	}

	// Other plugins can manipulate the list of widgets. Possibly to add recommended widgets
	return apply_filters('siteorigin_panels_widgets', $widgets);
}
