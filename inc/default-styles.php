...
function siteorigin_panels_default_styles_register_scripts(){
	wp_register_script('siteorigin-panels-front-styles', plugin_dir_url(SITEORIGIN_PANELS_BASE_FILE) . 'js/styling.min.js', array('jquery'), SITEORIGIN_PANELS_VERSION );
}
...
class SiteOrigin_Panels_Default_Styling {
	...
	static function init() {
		...
		add_filter('siteorigin_panels_row_style_fields', array(get_called_class(), 'row_style_fields' ) );
		add_filter('siteorigin_panels_widget_style_fields', array(get_called_class(), 'widget_style_fields' ) );

		// Filter the row style
		add_filter('siteorigin_panels_row_style_attributes', array(get_called_class(), 'row_style_attributes' ), 10, 2);
		add_filter('siteorigin_panels_cell_style_attributes', array(get_called_class(), 'cell_style_attributes' ), 10, 2);
		add_filter('siteorigin_panels_widget_style_attributes', array(get_called_class(), 'widget_style_attributes' ), 10, 2);

		// Main filter to add any custom CSS.
		add_filter('siteorigin_panels_css_object', array(get_called_class(), 'filter_css_object' ), 10, 3);

		// Filtering specific attributes
		add_filter('siteorigin_panels_css_row_margin_bottom', array(get_called_class(), 'filter_row_bottom_margin' ), 10, 2);
		add_filter('siteorigin_panels_css_row_gutter', array(get_called_class(), 'filter_row_gutter' ), 10, 2);
	}
	...
	static function row_style_attributes( $attributes, $args ) {
		...
		if( !empty( $args['background_color'] ) ) {
			$attributes['style'] .= 'background-color:' . $args['background_color']. ';';
		}
